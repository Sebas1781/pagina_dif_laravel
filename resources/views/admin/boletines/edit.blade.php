...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        <form method="POST" action="{{ route('admin.boletines.update', $boletin) }}" enctype="multipart/form-data" novalidate>
            ...
            {{-- URL de referencia --}}
            <div class="mb-5">
                <label for="url_referencia" class="block text-sm font-medium text-gray-700 mb-1.5">
                    Link de referencia / Más información
                    <span class="text-gray-400 font-normal">(opcional)</span>
                </label>
                <input type="url" id="url_referencia" name="url_referencia" value="{{ old('url_referencia', $boletin->url_referencia) }}"
                       class="w-full px-4 py-2.5 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-dif-pink focus:border-transparent
                              {{ $errors->has('url_referencia') ? 'border-red-400 bg-red-50' : 'border-gray-300' }}"
                       placeholder="https://ejemplo.com/noticia">
                @error('url_referencia')
                    <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
