...
@section('title', 'Editar Documento CONAC')
@section('page-title', 'Editar Documento CONAC')
...
<div class="max-w-2xl">
    ...
    <div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
        <a href="{{ route('admin.conac.index') }}" class="hover:text-dif-pink transition-colors">Documentos CONAC</a>
        <i class="fas fa-chevron-right text-xs text-gray-300"></i>
        <span class="text-dif-dark font-medium">Editar</span>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        <form method="POST" action="{{ route('admin.conac.update', $conac) }}" enctype="multipart/form-data" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="mb-5">
                ...
                <input type="number" id="anio" name="anio" value="{{ old('anio', $conac->anio) }}" min="2000" max="2099"
            ...
            <div class="mb-5">
                ...
                <input type="text" id="categoria" name="categoria" value="{{ old('categoria', $conac->categoria) }}" list="lista-categorias"
                       class="w-full px-4 py-2.5 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-dif-pink focus:border-transparent
            ...
            <div class="mb-5">
                ...
                <input type="text" id="nombre" name="nombre" value="{{ old('nombre', $conac->nombre) }}"
                       class="w-full px-4 py-2.5 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-dif-pink focus:border-transparent
            ...
            {{-- Info actual --}}
            @if($conac->tieneArchivo() || $conac->tieneLink())
                <div class="mb-5 p-3 rounded-xl bg-gray-50 border border-gray-200 text-sm">
                    ...
                    @if($conac->tieneArchivo())
                        <a href="{{ $conac->url }}" target="_blank" class="text-blue-600 hover:underline flex items-center gap-1">
                            <i class="fas fa-file-pdf text-dif-pink"></i> {{ basename($conac->archivo) }}
                        </a>
                    @else
                        <a href="{{ $conac->link_externo }}" target="_blank" class="text-purple-600 hover:underline flex items-center gap-1">
                            <i class="fas fa-link text-purple-500"></i> {{ \Illuminate\Support\Str::limit($conac->link_externo, 60) }}
                        </a>
                    @endif
                </div>
            @endif
            ...
            {{-- Tipo de fuente --}}
            @php
                $tipoActual = old('tipo_fuente', $conac->tieneArchivo() ? 'archivo' : 'link');
            @endphp
            <div class="mb-5">
                ...
                <div id="fuente-archivo" class="{{ $tipoActual === 'archivo' ? '' : 'hidden' }}">
                    ...
                    <p class="mt-1 text-xs text-gray-400">Dejar vacío para conservar el archivo actual. Solo PDF. Máx. 10 MB.</p>
                    @error('archivo')
                        <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div id="fuente-link" class="{{ $tipoActual === 'link' ? '' : 'hidden' }}">
                    <input type="url" id="link_externo" name="link_externo" value="{{ old('link_externo', $conac->link_externo) }}"
            ...
            <div class="grid grid-cols-2 gap-5 mb-6">
                <div>
                    ...
                    <input type="number" id="orden" name="orden" value="{{ old('orden', $conac->orden) }}" min="0"
                           class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-dif-pink focus:border-transparent">
                </div>
                <div>
                    ...
                    <label class="flex items-center gap-3 mt-3 cursor-pointer">
                        <input type="checkbox" name="activo" value="1"
                               {{ old('activo', $conac->activo) ? 'checked' : '' }}
