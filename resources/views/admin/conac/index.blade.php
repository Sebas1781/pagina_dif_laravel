...
@section('title', 'Documentos CONAC')
@section('page-title', 'Documentos CONAC')
...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <p class="text-sm text-gray-500 mt-0.5">Gestiona los documentos del Consejo Nacional de Armonización Contable.</p>
        </div>
        <a href="{{ route('admin.conac.create') }}"
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-5">
        <form method="GET" action="{{ route('admin.conac.index') }}" class="flex flex-wrap items-end gap-4">
            <div>
            ...
            <div>
                ...
                <input type="text" name="categoria" value="{{ request('categoria') }}" placeholder="Ej. Norma, Criterio..."
            ...
            <div>
                ...
                <input type="text" name="buscar" value="{{ request('buscar') }}" placeholder="Ej. Postulado, Marco..."
                       class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-dif-pink focus:border-transparent w-56">
            </div>
            <div class="flex gap-2">
                ...
                @if(request()->hasAny(['anio', 'categoria', 'buscar']))
                    <a href="{{ route('admin.conac.index') }}" class="px-4 py-2 text-sm font-medium text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                        Limpiar
                    </a>
                @endif
            </div>
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                @endif
                                <a href="{{ route('admin.conac.edit', $doc) }}"
                                ...
                                <form method="POST" action="{{ route('admin.conac.destroy', $doc) }}"
                                      onsubmit="return confirm('¿Eliminar este documento CONAC?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-5 py-16 text-center text-gray-400">
                            ...
                            No hay documentos CONAC registrados aún.
                        </td>
                    </tr>
                @endforelse
            </tbody>
