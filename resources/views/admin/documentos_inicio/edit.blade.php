...
@section('title', 'Editar PDF de Inicio')
@section('page-title', 'Editar PDF de Inicio')
...
<div class="max-w-2xl">
    ...
    <div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
        <a href="{{ route('admin.documentos_inicio.index') }}" class="hover:text-dif-pink transition-colors">PDFs de Inicio</a>
        <i class="fas fa-chevron-right text-xs text-gray-300"></i>
        <span class="text-dif-dark font-medium">Editar</span>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        <form method="POST" action="{{ route('admin.documentos_inicio.update', $documento) }}" enctype="multipart/form-data" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="mb-5">
                <label for="nombre" class="block text-sm font-medium text-gray-700 mb-1.5">
                    Nombre del botón <span class="text-red-500">*</span>
                </label>
                <input type="text" id="nombre" name="nombre" value="{{ old('nombre', $documento->nombre) }}"
                       ...
                       placeholder="Ej. Programa Anual de Desarrollo Archivístico 2026">
                <p class="mt-1 text-xs text-gray-400">Este texto se mostrará como etiqueta del botón en la página de inicio.</p>
                @error('nombre')
            ...
            {{-- Info actual --}}
            @if($documento->tieneArchivo() || $documento->tieneLink())
                <div class="mb-5 p-3 rounded-xl bg-gray-50 border border-gray-200 text-sm">
                    ...
                    @if($documento->tieneArchivo())
                        <a href="{{ $documento->url }}" target="_blank" class="text-blue-600 hover:underline flex items-center gap-1">
                            <i class="fas fa-file-pdf text-dif-pink"></i> {{ basename($documento->archivo) }}
                        </a>
                    @else
                        <a href="{{ $documento->link_externo }}" target="_blank" class="text-purple-600 hover:underline flex items-center gap-1">
                            <i class="fas fa-link text-purple-500"></i> {{ \Illuminate\Support\Str::limit($documento->link_externo, 60) }}
                        </a>
                    @endif
                </div>
            @endif
            ...
            {{-- Tipo de fuente --}}
            @php
                $tipoActual = old('tipo_fuente', $documento->tieneArchivo() ? 'archivo' : 'link');
            @endphp
            <div class="mb-5">
                ...
                <div id="fuente-link" class="{{ $tipoActual === 'link' ? '' : 'hidden' }}">
                    <input type="url" id="link_externo" name="link_externo" value="{{ old('link_externo', $documento->link_externo) }}"
            ...
            <div class="grid grid-cols-2 gap-5 mb-6">
                <div>
                    ...
                    <input type="number" id="orden" name="orden" value="{{ old('orden', $documento->orden) }}" min="0"
                           class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-dif-pink focus:border-transparent">
                    <p class="mt-1 text-xs text-gray-400">Menor número = aparece primero.</p>
                </div>
                <div>
