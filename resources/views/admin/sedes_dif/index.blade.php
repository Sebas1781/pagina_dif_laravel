...
@section('title', 'Sedes DIF Tecamac')
@section('page-title', 'Sedes DIF Tecamac')

@section('content')
<div class="flex items-center justify-between mb-6">
    <p class="text-sm text-gray-500">Gestiona las sedes del apartado Nosotros.</p>
    <a href="{{ route('admin.sedes_dif.create') }}"
    ...
    <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-100">
            <tr>
                ...
                <th class="px-5 py-3.5 text-left font-semibold text-gray-600 w-32">Icono</th>
                <th class="px-5 py-3.5 text-left font-semibold text-gray-600 w-32">Color</th>
        ...
        <tbody class="divide-y divide-gray-50">
            @forelse($items as $item)
                <tr class="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td class="px-5 py-4 text-xs text-gray-500"><i class="fas {{ $item->icono }} mr-2"></i>{{ $item->icono }}</td>
                    <td class="px-5 py-4 text-xs text-gray-500">{{ $item->color }}</td>
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('admin.sedes_dif.edit', $item) }}"
                            ...
                            <form method="POST" action="{{ route('admin.sedes_dif.destroy', $item) }}" onsubmit="return confirm('¿Eliminar esta sede?')">
                                @csrf @method('DELETE')
                                <button type="submit"
            ...
            @empty
                <tr><td colspan="7" class="px-5 py-16 text-center text-gray-400"><i class="fas fa-map-location-dot text-4xl mb-3 block"></i>No hay sedes registradas.</td></tr>
            @endforelse
        </tbody>
    </table>
    @if($items->hasPages())
