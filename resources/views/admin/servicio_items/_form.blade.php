<div class="grid grid-cols-2 gap-5 mb-5">
    <div>
        ...
        <select id="categoria" name="categoria"
                class="w-full px-4 py-2.5 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-dif-pink focus:border-transparent {{ $errors->has('categoria') ? 'border-red-400 bg-red-50' : 'border-gray-300' }}">
            @foreach($categorias as $clave => $nombre)
                <option value="{{ $clave }}" {{ old('categoria', $item->categoria ?? '') === $clave ? 'selected' : '' }}>{{ $nombre }}</option>
            @endforeach
        </select>
        @error('categoria') <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p> @enderror
    </div>
    <div>
...
<div class="mb-5">
    <label for="nombre" class="block text-sm font-medium text-gray-700 mb-1.5">Elemento <span class="text-red-500">*</span></label>
