...
<div class="mb-5">
    <label for="descripcion" class="block text-sm font-medium text-gray-700 mb-1.5">Descripcion / Direccion</label>
    <textarea id="descripcion" name="descripcion" rows="2"
              class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm resize-none focus:outline-none focus:ring-2 focus:ring-dif-pink focus:border-transparent">{{ old('descripcion', $item->descripcion ?? '') }}</textarea>
</div>

<div class="mb-5">
    <label for="horario" class="block text-sm font-medium text-gray-700 mb-1.5">Horario</label>
    <input type="text" id="horario" name="horario" value="{{ old('horario', $item->horario ?? '') }}"
...
<div class="mb-5">
    <label class="block text-sm font-medium text-gray-700 mb-1.5">Color del horario</label>
    <p class="text-xs text-gray-500 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2">
        El color del horario se asigna automaticamente de forma aleatoria al crear el servicio.
    </p>
</div>

<div class="mb-5">
    <label class="block text-sm font-medium text-gray-700 mb-1.5">Imagen <span class="text-gray-400 font-normal">(jpg, png, webp - max. 2 MB)</span></label>
    @if(isset($item) && $item->imagen)
        @php
            $src = str_starts_with($item->imagen, 'servicios_salud/')
