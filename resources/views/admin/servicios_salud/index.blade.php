...
@section('title', 'Servicios de Salud (Inicio)')
@section('page-title', 'Servicios de Salud (Inicio)')

@section('content')
<div class="flex items-center justify-between mb-6">
    <p class="text-sm text-gray-500">Gestiona las tarjetas del apartado Servicios de Salud en Inicio.</p>
    <a href="{{ route('admin.servicios_salud.create') }}"
    ...
    <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-100">
            <tr>
                ...
                <th class="px-5 py-3.5 text-left font-semibold text-gray-600">Descripcion</th>
                <th class="px-5 py-3.5 text-left font-semibold text-gray-600">Horario</th>
        ...
        <tbody class="divide-y divide-gray-50">
            @forelse($items as $item)
                <tr class="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td class="px-5 py-4">
                        @php
                            $src = $item->imagen
                                ? (str_starts_with($item->imagen, 'servicios_salud/')
                    ...
                    <td class="px-5 py-4 text-xs {{ $item->color_horario ?: 'text-dif-pink' }} font-medium max-w-xs">
                        <p class="line-clamp-2">{{ $item->horario }}</p>
                    </td>
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('admin.servicios_salud.edit', $item) }}"
                            ...
                            <form method="POST" action="{{ route('admin.servicios_salud.destroy', $item) }}"
            ...
            @empty
                <tr><td colspan="7" class="px-5 py-16 text-center text-gray-400">
                    <i class="fas fa-notes-medical text-4xl mb-3 block"></i> No hay servicios registrados.
                </td></tr>
            @endforelse
        </tbody>
    </table>
