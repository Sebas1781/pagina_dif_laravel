...
<body class="font-sans text-dif-gray bg-white overflow-x-hidden">
                    ...
                    <div class="relative group">
                        ...
                        <div class="absolute top-full left-0 mt-1 w-72 bg-white rounded-xl shadow-xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                            ...
                            <div class="border-t border-gray-100">
                                ...
                                <div id="nav-conac" class="hidden border-t border-gray-100 bg-gray-50 overflow-hidden">
                                    ...
                                    @empty
                                        <span class="block px-8 py-2.5 text-sm text-gray-400 italic">Sin documentos aún</span>
                                    @endforelse
                                </div>
                            </div>
                            {{-- Presupuesto nested submenu --}}
                            <div class="border-t border-gray-100">
                                <button onclick="toggleNavSubmenu('nav-presupuesto', this)"
                                    ...
                                    <span><i class="fas fa-coins mr-2 text-dif-pink"></i>Presupuesto</span>
                                    <i class="fas fa-chevron-down text-xs text-gray-400 transition-transform duration-300"></i>
                                </button>
                                <div id="nav-presupuesto" class="hidden border-t border-gray-100 bg-gray-50 rounded-b-xl overflow-hidden">
                                    @forelse($presupuestoAniosNav as $anio)
                                        <a href="{{ route('transparencia') }}#presupuesto-{{ $anio }}" class="block px-8 py-2.5 text-sm text-dif-dark hover:bg-dif-cream hover:text-dif-pink {{ $loop->last ? 'rounded-b-xl' : '' }} transition-colors">Presupuesto {{ $anio }}</a>
                                    @empty
            ...
            <div class="px-4 py-4 space-y-1 max-h-[calc(100vh-5rem)] overflow-y-auto">
                ...
                <div>
                    ...
                    <div class="hidden pl-6 space-y-1 mt-1">
                        ...
                        <div>
                            <button onclick="this.nextElementSibling.classList.toggle('hidden')" class="w-full flex items-center justify-between px-4 py-2 rounded-lg text-sm text-white/80 hover:bg-white/10 hover:text-white">
                                <span><i class="fas fa-coins mr-2 text-dif-pink-light"></i>Presupuesto</span>
                                <i class="fas fa-chevron-down text-xs"></i>
                            </button>
                            <div class="hidden pl-4 space-y-1 mt-1">
                                @forelse($presupuestoAniosNav as $anio)
                                    <a href="{{ route('transparencia') }}#presupuesto-{{ $anio }}" class="block px-4 py-2 rounded-lg text-xs text-white/70 hover:bg-white/10 hover:text-white">Presupuesto {{ $anio }}</a>
                                @empty
                                    <span class="block px-4 py-2 text-xs text-white/40 italic">Sin documentos aún</span>
                                @endforelse
                            </div>
    ...
    <script>
        ...
        const navSubmenus = ['nav-sevac', 'nav-conac', 'nav-presupuesto'];
        function toggleNavSubmenu(id, btn) {
            const menu = document.getElementById(id);
            const icon = btn.querySelector('.fa-chevron-down');
            const isHidden = menu.classList.toggle('hidden');
            icon.classList.toggle('rotate-180', !isHidden);
            // Cerrar los otros si están abiertos
            navSubmenus.filter(s => s !== id).forEach(other => {
